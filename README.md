# leetcode-practice
practice for google May 11

# LeetCode Practice for Google Interview

## Problem Bank (15 problems)

### Tier 1 — Design Problems (OOP focus, highest priority)
- [ ] 146 - LRU Cache
- [ ] 362 - Design Hit Counter
- [ ] 359 - Logger Rate Limiter
- [ ] 348 - Design Tic-Tac-Toe
- [ ] 1600 - Design Phone Directory

### Tier 2 — Pattern Fundamentals
- [ ] 347 - Top K Frequent Elements
- [ ] 49 - Group Anagrams
- [ ] 3 - Longest Substring Without Repeating Characters
- [ ] 207 - Course Schedule
- [ ] 139 - Word Break

### Tier 3 — OOP-flavored, Real-world
- [ ] 1396 - Design Underground System
- [ ] 1472 - Design Browser History
- [ ] 1797 - Snapshot Array
- [ ] 355 - Design Twitter
- [ ] 981 - Time Based Key-Value Store

## Scoring Rubric (what Claude evaluates on)
- Class design (sensible boundaries, good naming)
- Variable naming (clarity, no cryptic abbreviations)
- Edge case handling (mentioned or coded)
- Tradeoff articulation (why this design choice)
- Code cleanliness (30-50 lines, no fluff)

## Notes
- No AI assist during solve
- No execution/testing — design focus
- Talk out loud the whole time
- Post-solve: write tradeoffs in notes.md